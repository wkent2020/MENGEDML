...
'''
This framework should work for shapes that are not rectangles, so long as 
...
def rectangle(x, y, z, semiaxes):
    '''
    Characterisitic function of a rectangular prism with semiaxes given and aligned along x,y,z axes
    '''
    ...
    if np.amax(np.abs(np.array([x,y,z])/semiaxes)) <= 1:
        return 1
    else:
        return 0
...
def generalRectangle(x, y, z, semiaxes, rotation, translation):
    '''
    Characteristic function for a general rectangular prism
    ...
    return rectangle(xNew, yNew, zNew, semiaxes)
...
for yz in yzCoordinates:
    ...
    projectedThickness, err = integrate.quad(generalRectangle, minCoordinate[0], maxCoordinate[0], args=(yz[0], yz[1], semiaxes, r, translation))
...
ax.set_title("Rectangle with axes " + str(semiaxes[0]) + "," + str(semiaxes[1]) + "," + str(semiaxes[2]))
